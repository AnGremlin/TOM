  ...
  setText(text: string, wrapWidth?: number) {
    ...
    while (index < Math.max(this.letterActors.length, this.text.length)) {
      ...
      if (index >= this.letterActors.length) {
        ...
        new Sup.SpriteRenderer(letterActor, Sup.get("TomE/HUD/Text/Font", Sup.Sprite))
      ...
      //toggle italics if needed
      if (this.italicsInidces.indexOf(index) != -1) {
        this.italicsOn = !this.italicsOn;
      }
      //if italics is on grab italics char
      if (this.italicsOn) {
        code += this.italicsOffset;
      }
